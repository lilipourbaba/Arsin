import { websiteActivate } from "./custom-events";

export const deActivateEl = (nodeEl) => {
  nodeEl.setAttribute("data-active", "false");
  nodeEl.dispatchEvent(websiteActivate);
};
...
export const definePopUp = (nodeEl) => {
  nodeEl.addEventListener("websiteActivate", (e) => {
...
export const RemoveSwiperClass = (selector) => {
 ...
 *
 * @use RemoveSwiperClass
 *
 * @param {Swiper} swiper
 * @param {string} selector
 * @return {void}
 */
export const DestroySwiper = (swiper, selector) => {
  const swiperEl = document.querySelector(selector);
  if (!swiperEl) return;
  swiper.destroy(true, true);
  RemoveSwiperClass(selector);
};
